...
index e8768ce726ab878ff69405e495c626ec864fe187..5829845a1f3bfb9fe909b5eeb06e11bcedc95324 100644
--- a/src/main.rs
+++ b/src/main.rs
@@ -1,45 +1,46 @@
...
@@ -58,354 +59,440 @@ struct CommitRequest {
 #[derive(Debug, Serialize)]
 #[serde(tag = "op")]
...
+                wait_for_child_shutdown(&mut child).await;
             ...
             status = child.wait() => {
                 match status {
-                    Ok(status) => eprintln!("child exited with status: {status}"),
-                    Err(err) => eprintln!("failed waiting for child: {err:#}"),
+                    Ok(status) => {
...
-    let mut parts = cfg.dataset_id.split('/');
-    let owner = parts
-        .next()
...
+            size,
         ...
         let changed = state
             .files
             .get(&relative)
-            .map(|s| s.md5 != md5 || s.size != bytes.len() as u64)
